                    ...
                    <div class="card-header pb-0">
                        <h4 class="card-title">
                            <div class="d-flex justify-content-between">
                                <div>Notice Lists</div>
                                <div><a href="{{route('admin.notice.create')}}" class="btn btn-primary btn-sm"> <i class="bx bx-plus"></i> Add New Item </a> </div>
                            </div>
                        </h4>
                    </div>
                                ...
                                <table class="table table-striped dataex-html5-selectors">
                                    ...
                                    @foreach($datas as $key => $row)
                                        <tr>
                                            ...
                                            <td>{{$row->status}}</td>
                                            <td>
                                                <a href="{{route('admin.notice.create', $row->id)}}"
                                                   class="btn btn-warning" style="padding: 3px 7px;font-size: 20px" data-toggle="tooltip" title='Edit'>
                                                    <i class="bx bx-pencil"></i></a>
                                                <form method="POST" action="{{route('admin.notice.delete', $row->id)}}"
                                                      class="d-inline">@csrf
                                                    {{method_field('DELETE')}}
                                                    <button type="submit"
