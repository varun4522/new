...
try {
    ...
    // Prefer reading `duration_hours` from `packages` table (if present), fallback to static mapping
    $maturityHours = null;
    try {
        $stmt = $conn->prepare("SELECT duration_hours, validity FROM packages WHERE price = ? LIMIT 1");
        if ($stmt) {
            ...
            if ($res && $res->num_rows > 0) {
                ...
                if (!empty($row['duration_hours'])) {
                    $maturityHours = (int)$row['duration_hours'];
                } elseif (!empty($row['validity'])) {
                    // `validity` historically stored as days; convert to hours
                    $maturityHours = (int)$row['validity'] * 24;
                }
            }
            $stmt->close();
        }
    } catch (Exception $e) {
        // ignore DB lookup errors and fallback to static mapping below
        error_log('invest.php: package lookup failed: ' . $e->getMessage());
    }

    if (empty($maturityHours)) {
        // fallback to the static mapping included in this API
        $maturityHours = (int)$plan['hours'];
    }

    $maturityDate = date('Y-m-d H:i:s', strtotime("+{$maturityHours} hours"));
